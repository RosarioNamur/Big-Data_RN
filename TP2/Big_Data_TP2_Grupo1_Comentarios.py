...
El único tema fue lo del bandwidth de los kernesl. No pasa nada!!!
Felicitaciones!!!
"""
...
os.chdir("/Users/tomaspacheco/Documents/GitHub/E337-Grupo1/TP2/")
...
### COMENTARIO TOMI: esto era exactamente lo que se pretendia que hagan!
...
import pandas as pd
import numpy as np
### COMENTARIO TOMI: pandas ya estaba importado. Lo mejor es siempre importar todos los paquetes en el preambulo
...
### COMENTARIO TOMI: no exporten tantas bases intermedias
...
## COMENTARIO TOMI: esto esta definido en el lugar incorrecto, porque lo usan antes.
#vars_jerarquico = ["ch04","ch07", "nivel_ed"]
...
##### Ej c #####
#Somos conscientes del equilibrio entre cumplir estrictamente con una consigna mediante herramientas automatizadas 
# y, por otro lado, profundizar en el aprendizaje de los conceptos discutidos en clase. 
# En este trabajo, optamos por priorizar nuestro proceso de aprendizaje, aun cuando ello pueda implicar una 
# calificación más baja. Consideramos que este enfoque es más enriquecedor y coherente con
# los objetivos formativos del curso.
...
for año in [2004, 2024]:
    ...
    for estado in [1, 0]:
        ...
        if len(datos) > 0:
            x, y = estimar_densidad_plot(datos.values, bandwidth=50000)
            ax.plot(x, y, alpha=0.7, label=labels[estado], color=colores[estado])
            
            # El desvio es bastante grande para un bw de 500
            print(np.var(datos.values))
...
"""
COMENTARIO TOMI: creo que lo que estaba pasando es que tenian el bandwidth un poco chico (500)
y lo primero que hice fue subirlo a 50.000 y con eso funcionó mejor.
#
...
bandwidths = np.linspace(50000, 1000000, 200)  # Ajustá el rango según tu escala


""" COMENTARIO TOMI
Este fue el segundo problema. Pusieron un bw máximo de 2000
