...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr 12 17:26:06 2025
...
# Esta función convierte la columna a string (por si hay "00" como texto) 
# y luego reemplaza los valores indicados por NaN
def reemplazar_por_nan(df, variable, valores_a_reemplazar):
    df[variable] = df[variable].astype(str)  # convierte a string para que "00" funcione
    # convierte también los valores a string si son enteros o listas de enteros
    if isinstance(valores_a_reemplazar, list):
        valores_a_reemplazar = [str(v) for v in valores_a_reemplazar]
    else:
        valores_a_reemplazar = [str(valores_a_reemplazar)]
    df[variable] = df[variable].replace(valores_a_reemplazar, np.nan)
...
reemplazar_por_nan(BaseAppend1, "ch14", [98, 99, 0, "00"])  # agregamos 0 y "00"
...
BaseAppend1['ch12'] = BaseAppend1['ch12'].replace({'Jardí­n/Preescolar': 1, 'Primario': 2, 'EGB': 3, "Secundario" : 4, "Polimodal" : 5, 'Terciario':6,  "Universitario": 7, "Posgrado Universitario":8, "Educación especial (discapacitado)":9 })
BaseAppend1['ch14'] = BaseAppend1["ch14"].replace({  '01': 1, '02': 2, '03': 3, '04' : 4, '05' : 5, '06': 6, '07': 7, '08': 8, '09': 9  })
...
# $1=$462,37 un peso de 2004 son 462 de 2024

BaseAppend1['ipcf_2024'] = df['ipcf']

BaseAppend1.loc[df['ano4'] == 2004, 'ipcf_2024'] = df['ipcf'] * 462.37

BaseAppend1[['ipcf', 'ipcf_2024', 'ano4']].head()


##Guardamos los cambios realizados 
BaseAppend1.to_excel("Base_EPH_Limpia.xlsx", index=False)
